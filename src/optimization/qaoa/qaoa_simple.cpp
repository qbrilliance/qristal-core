...
int QaoaSimple::is_ii_consistent() {
  ...
  if ((N_ii = singleton_or_eqlength(thetas_, N_ii)) == INVALID) {
    ...
    // if ((N_ii = singleton_or_eqlength(ansatzs_, N_ii)) == INVALID) {
    // std::cout << "[ansatz] shape is invalid" << std::endl;
    // return INVALID;
  // }
...
/*  
  auto noiseModel = std::make_shared<qb::NoiseModel>();
...
//                                      {"noise-model", noiseModel->to_json()}});
  ...
  // auto optx = (*buffer)["opt-params"].as<std::vector<double>>(); 
  // std::reverse(optx.begin(), optx.end());
