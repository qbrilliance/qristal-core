...
inline std::string convertRawOpenQasmToQBKernel(const std::string &in_rawQasm) {
  ...
  const std::regex qregDeclarationRegex(
      R"(qreg[ \t]+([a-zA-Z_$][a-zA-Z_$0-9]*)\[\d+\]\s*;)");
    std::smatch matches;
...
namespace qbOS {
/// Core Qbqe methods: run/run_async executions and helpers for compilation/post-processing...
Qbqe::circuit_input_types Qbqe::validate_infiles_instrings_randoms_irtarget_ms_nonempty(const int &ii,
                                                      const int &jj) {
  Qbqe::circuit_input_types returnval = Qbqe::circuit_input_types::INVALID;
  if (debug_qbqe_) {
    std::cout
  ...
  if (!is_instrings_empty) {
    ...
    if (temp_instr.find("__qpu") == 0) {
      ...
      return Qbqe::circuit_input_types::VALID_INSTRING_QPU;
  ...
  if (!is_randoms_empty) {
    if (((randoms_[ii])[0] > 0) &&
        ((randoms_[ii])[0] <= Qbqe::RANDOMS_UPPERBOUND)) {
      returnval = Qbqe::circuit_input_types::VALID_RANDOM;
      return returnval;
    } else {
      // instring is a text description
      if (debug_qbqe_) {
        ...
        throw std::range_error("qbqe: random exceeds valid limits");
      }
    }
  }
  ...
  if (!is_infiles_empty) {
    ...
    if (tifs.is_open()) {
      ...
      return Qbqe::circuit_input_types::VALID_INFILE;
    } else {
  ...
  if (!is_irtarget_m_empty) {
      ...
      return Qbqe::Qbqe::circuit_input_types::VALID_IR;
  }

  return returnval;
}

int Qbqe::validate_instrings() {
...
int Qbqe::validate_sns_nonempty() {
  const int VALID_ALL = 0;
  // const int VALID_PARTIAL = 1;
  const int INVALID = -1;
  ...
  if (!sns_.empty()) {
    ...
    if (debug_qbqe_ && is_sns_empty) {
      std::cout << "[debug]:"
                << "sns_ is empty" << std::endl;
    }
  }
  if (is_sns_empty) {
    throw std::invalid_argument("qbqe: number of shots [sn] must have a value");
    returnval = INVALID;
  }
  return returnval;
}

int Qbqe::validate_qns_nonempty() {
  const int VALID_ALL = 0;
  // const int VALID_PARTIAL = 1;
  const int INVALID = -1;
  ...
  if (is_qns_empty) {
    throw std::invalid_argument(
        "qbqe: number of qubits [qn] must have a value");
    returnval = INVALID;
  }
  return returnval;
}

int Qbqe::validate_rns_nonempty() {
  const int VALID_ALL = 0;
  // const int VALID_PARTIAL = 1;
  const int INVALID = -1;
  ...
  if (is_rns_empty) {
    throw std::invalid_argument(
        "qbqe: number of repetitions [rn] must have a value");
    returnval = INVALID;
  }
  return returnval;
}

int Qbqe::validate_thetas_option() {
  const int VALID_ALL = 0;
  const int NOT_SET = -1;
  // const int INVALID = -2;
  ...
  if (!thetas_.empty()) {
    ...
    if (debug_qbqe_ && is_thetas_empty) {
      std::cout << "[debug]:"
...
template <class TT>
int Qbqe::singleton_or_eqlength(const TT &in_d, const int N_ii) {
  const int INVALID = -1;
...
template <class TT> int Qbqe::eqlength(const TT &in_d, const int N_ii) {
  const int INVALID = -1;
  if (in_d.size() == N_ii) {
    return N_ii;
  } else {
...
classical bit values.
  ...
  NONE
**/
std::string Qbqe::random_circuit(const int &n_q, const int &depth) {
 ...
 **/
template <typename TP> double Qbqe::get_probability(const TP &in_elem) {
  return in_elem;
}

// Specialise for complex double
template <> double Qbqe::get_probability(const std::complex<double> &in_elem) {
  return std::norm(in_elem);
}

template <>
double Qbqe::get_probability(
    const std::pair<const int, std::complex<double>> &in_pair) {
  return std::norm(in_pair.second);
}

template <typename TQ, class TV>
TQ Qbqe::get_key(const std::map<TQ, TV> &in_q, const int &key_in_q) {
  return (TQ)key_in_q;
}

template <class TT, class TV>
int Qbqe::get_key(const std::map<int, TT> &in_q,
...
template <>
std::string Qbqe::get_key(const std::map<std::string, int> &in_q,
                          const int &key_in_q) {
...
double Qbqe::get_jensen_shannon_divergence(const std::map<TQ, int> &in_q,
                                           const TP &in_p,
...
void Qbqe::get_jensen_shannon(const size_t &ii, const size_t &jj) {
  // Validations
  if (debug_qbqe_) {
...
void Qbqe::get_jensen_shannon() {
  if (debug_qbqe_) std::cout << "Invoked get_jensen_shannon()" << std::endl;
...
std::string Qbqe::aer_circuit_transpiler(std::string &circuit) {
...
void Qbqe::profile(const size_t &ii, const size_t &jj) {}

/// Retrieve and validate run configurations for index pair (ii, jj) using the table index convention.
Qbqe::run_i_j_config Qbqe::get_run_config(size_t ii, size_t jj)
{
  Qbqe::run_i_j_config config;
  /// Number of shots
  {
    ValidatorTwoDim<VectorN, size_t> sns_valid(sns_, Qbqe::SNS_LOWERBOUND, Qbqe::SNS_UPPERBOUND,
                                               " number of shots [sn] ");
  ...
  /// Number of qubits
  {
    ValidatorTwoDim<VectorN, size_t> qns_valid(qns_, Qbqe::QNS_LOWERBOUND, Qbqe::QNS_UPPERBOUND,
                                               " number of qubits [qn] ");
  ...
  {
    ValidatorTwoDim<VectorN, size_t> rns_valid(rns_, Qbqe::RNS_LOWERBOUND, Qbqe::RNS_UPPERBOUND,
  ...
  /// TNQVM-MPS simulator max bond dimension
  {
    ValidatorTwoDim<VectorN, size_t> max_bond_dimensions_valid(
        max_bond_dimensions_, Qbqe::MAX_BOND_DIMENSION_LOWERBOUND, Qbqe::MAX_BOND_DIMENSION_UPPERBOUND,
...
void Qbqe::ensure_results_table_size(size_t ii, size_t jj)
{
...
std::string Qbqe::get_target_circuit_qasm_string(size_t ii, size_t jj, const run_i_j_config& run_config) {
 ...
 const auto file_or_string_or_random_or_ir =
  ...
  } else if (file_or_string_or_random_or_ir == Qbqe::circuit_input_types::VALID_INSTRING_QPU) {
    // String input
    target_circuit = (instrings_[ii])[0];
  } else if (file_or_string_or_random_or_ir == Qbqe::circuit_input_types::VALID_RANDOM) {
  ...
  } else if (file_or_string_or_random_or_ir == Qbqe::circuit_input_types::VALID_IR) {
    if (debug_qbqe_) {
...
void Qbqe::run(const size_t &ii, const size_t &jj) {
  ...
  if (file_or_string_or_random_or_ir == Qbqe::circuit_input_types::INVALID) {
    ...
    if (debug_qbqe_) std::cout << "# Qubits: " << n_qubits << std::endl;
    ...
    if (debug_qbqe_) std::cout << "# Shots: " << shots << std::endl;

    size_t n_samples = run_config.num_repetitions;
    if (debug_qbqe_) std::cout << "# Repetitions: " << n_samples << std::endl;
    ...
    if (accs.compare("tnqvm") == 0) {
      ...
      if (aws_device_names.compare("DM1") == 0) {
        ValidatorTwoDim<VectorN, size_t> qns_dm1_valid(
            qns_, Qbqe::QNS_DM1_LOWERBOUND, Qbqe::QNS_DM1_UPPERBOUND,
        ...
        ValidatorTwoDim<VectorN, size_t> sns_dm1_valid(
            sns_, Qbqe::SNS_DM1_LOWERBOUND, Qbqe::SNS_DM1_UPPERBOUND,
      ...
      if (aws_device_names.compare("SV1") == 0) {
          ValidatorTwoDim<VectorN, size_t> qns_sv1_valid(
            qns_, Qbqe::QNS_SV1_LOWERBOUND, Qbqe::QNS_SV1_UPPERBOUND,
        ...
        ValidatorTwoDim<VectorN, size_t> sns_sv1_valid(
            sns_, Qbqe::SNS_SV1_LOWERBOUND, Qbqe::SNS_SV1_UPPERBOUND,
      ...
      if (aws_device_names.compare("TN1") == 0) {
          ValidatorTwoDim<VectorN, size_t> qns_tn1_valid(
            qns_, Qbqe::QNS_TN1_LOWERBOUND, Qbqe::QNS_TN1_UPPERBOUND,
        ...
        ValidatorTwoDim<VectorN, size_t> sns_tn1_valid(
            sns_, Qbqe::SNS_TN1_LOWERBOUND, Qbqe::SNS_TN1_UPPERBOUND,
    ...
    if (debug_qbqe_) std::cout << "# Quantum Brilliance accelerator: initialised" << std::endl;
...
void Qbqe::run(const size_t &ii) {}

void Qbqe::run() {
  if (debug_qbqe_) std::cout << "Invoked run()" << std::endl;
...
void Qbqe::set_parallel_run_config(const std::string &in_config) {
...
Executor &Qbqe::get_executor() { return *executor_; }

// !IMPORTANT! Make sure this function stays threadsafe
std::shared_ptr<async_job_handle> Qbqe::run_async(const std::size_t ii, const std::size_t jj,
  ...
  if (!run_config.no_sim)
  {
    try
    {
      ...
      if (qpu->name() == "aws_acc" && std::dynamic_pointer_cast<qbOS::remote_accelerator>(qpu))
      {
        auto as_remote_acc = std::dynamic_pointer_cast<qbOS::remote_accelerator>(qpu);
  ...
  void Qbqe::process_run_result(const std::size_t ii, const std::size_t jj, const run_i_j_config& run_config,
                                ...
                                std::shared_ptr<xacc::quantum::QuantumBrillianceAccelerator> qb_transpiler)
{
  ...
  if (run_config.oqm_enabled)
  {
    ...
    // Invoke the Profiler
    Profiler timing_profile(qb_transpiler->getTranspiledResult(), run_config.num_qubits, debug_qbqe_);
