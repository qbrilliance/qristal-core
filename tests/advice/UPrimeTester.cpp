...
TEST(UPrimeTester_1, checkSimple) {
  ...
  //auto u_prime = std::dynamic_pointer_cast<xacc::quantum::UPrime>(
    //  xacc::getService<xacc::Instruction>("UPrime"));
  std::shared_ptr<xacc::quantum::UPrime> u_prime =
      std::make_shared<xacc::quantum::UPrime>();
  std::cout << "expand\n";
...
TEST(UPrimeTester_2, checkSimple) {
  ...
  std::shared_ptr<xacc::quantum::UPrime> u_prime =
      std::make_shared<xacc::quantum::UPrime>();
  //auto u_prime = xacc::getService<xacc::CompositeInstruction>("UPrime");
  const bool expand_ok = u_prime->expand(
      {{"qubits_ancilla_metric", qubits_ancilla_prob}, {"qubits_ancilla_letter", qubits_ancilla_letter},
<<<<<<< HEAD
        {"qubits_next_letter_metric", qubits_next_letter_probabilities},{"qubits_next_letter", qubits_next_letter}});
=======
        {"qubits_next_letter_metric", qubits_next_letter_probabilities},{"qubits_next_letter", qubits_next_letter},});
>>>>>>> main
  EXPECT_TRUE(expand_ok);
