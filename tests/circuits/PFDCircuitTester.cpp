...
TEST(PFDCircuitTester, CheckGrid) {
  ...
  for (int i = 0; i < 8; i++) {
    for (int j = i+1; j < 8; j++) {
      ...
      std::vector<int> qubits_ancilla; // Length = 2*precision + 1
      for (int k = 9; k < 16; k++) {
      ...
      const bool expand_ok = pfd->expand({{"qubits_numerator", qubits_numerator},
      ...
      std::string expected_output = numerator_bin + denominator_bin + classical_result_string;
      for (auto bit : qubits_ancilla) {
          expected_output += '0';
      }
