...
def test_SPAM_correction_random():
  ...
  s.SPAM_confusion_matrix = confusion
  s.run()

  #(4) Obtain native and SPAM-corrected <ZZ>
  native_ZZ = 0.0
  for key in s.results_native:
    if key[0] == key[1]:
      native_ZZ += s.results_native[key] / s.results_native.total_counts()
    else:
      native_ZZ -= s.results_native[key] / s.results_native.total_counts()
  corrected_ZZ = 0.0
  for key in s.results:
    if key[0] == key[1]:
      corrected_ZZ += s.results[key] / s.results.total_counts()
    else:
      corrected_ZZ -= s.results[key] / s.results.total_counts()
