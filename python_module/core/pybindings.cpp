...
PYBIND11_MODULE(core, m) {
  ...
  py::bind_vector<N>(m, "N")
    .def("__repr__", [](const N &a) {
        json jret = a;
        return jret.dump();
    });
  ...
  py::class_<qb::KrausOperator>(m, "KrausOperator")
      ...
      .def_readwrite("qubits", &qb::KrausOperator::qubits, R"(Qubits that this Kraus operator acts on.)");

  py::class_<qb::AmplitudeDampingChannel>(m, "AmplitudeDampingChannel", R"(Amplitude damping channel factory)")
      .def(py::init<>())
      .def_readonly_static("name", &qb::AmplitudeDampingChannel::name)
      .def("Create", &qb::AmplitudeDampingChannel::Create);

  py::class_<qb::PhaseDampingChannel>(m, "PhaseDampingChannel", R"(Phase damping channel factory)")
      .def(py::init<>())
      .def_readonly_static("name", &qb::PhaseDampingChannel::name)
      .def("Create", &qb::PhaseDampingChannel::Create);

  py::class_<qb::DepolarizingChannel>(m, "DepolarizingChannel", R"(Depolarizing channel factory)")
      .def(py::init<>())
      .def_readonly_static("name", &qb::DepolarizingChannel::name)
      .def("Create", py::overload_cast<size_t, double>(&qb::DepolarizingChannel::Create), R"(
      ...
      .def("Create", py::overload_cast<size_t, size_t, double>(&qb::DepolarizingChannel::Create), R"(
  ...
  py::class_<qb::GeneralizedPhaseAmplitudeDampingChannel>(m, "GeneralizedPhaseAmplitudeDampingChannel", R"(Generalized Single-qubit combined phase and amplitude damping quantum error channel)")
      .def(py::init<>())
      .def_readonly_static("name", &qb::GeneralizedPhaseAmplitudeDampingChannel::name)
  ...
  py::class_<qb::GeneralizedAmplitudeDampingChannel>(m, "GeneralizedAmplitudeDampingChannel", R"(Generalized amplitude damping quantum error channel)")
      .def(py::init<>())
      .def_readonly_static("name", &qb::GeneralizedAmplitudeDampingChannel::name)
  ...
  py::class_<qb::NoiseProperties>(m, "NoiseProperties", R"(
    ...
    Use NoiseProperties to accept user input parameters for custom noise models.  There are 3 types of inputs used for constructing a custom noise model:
      ...
      .def_readwrite("gate_pauli_errors", &qb::NoiseProperties::gate_pauli_errors, R"(
  ...
  py::class_<qb::ReadoutError>(m, "ReadoutError", R"(Probabilities of reading out a value for a qubit that does not reflect its true state.)")
      .def(py::init<>())
      .def_readwrite("p_01", &qb::ReadoutError::p_01, R"(Classical probability of detecting 0 whereas the true state was :math:`|1\rangle`)")
      .def_readwrite("p_10", &qb::ReadoutError::p_10, R"(Classical probability of detecting 1 whereas the true state was :math:`|0\rangle`)");


  py::class_<qb::NoiseModel> nm(m, "NoiseModel", R"(Noise model class allowing specification of noise parameters and connectivity for each gate.)");
      nm.def(py::init<>())
        ...
        .def(py::init<const std::string&, 
                      ...
                      std::optional<std::reference_wrapper<const std::vector<std::pair<size_t, size_t>>>>>(),
             py::arg("name"), 
             py::arg("nb_qubits"),
             py::arg("connectivity") = std::nullopt,
             py::arg("connected_pairs") = std::nullopt)
        .def("to_json", &qb::NoiseModel::to_json, R"(Convert noise model to json string)")
        ...
        // Note that this can be removed when the silly aliases in typedefs.hpp are deleted.
        .def("add_gate_error", [](qb::NoiseModel* model, const NoiseChannel &noise_channel, const std::string &gate_name, const py::list qubits)
          {
            std::vector<size_t> v;
            for (auto& x : qubits) v.emplace_back(py::cast<size_t>(x));
            model->add_gate_error(noise_channel, gate_name, v);
          }, 
          R"(        Overload of add_gate_error that takes *qubits* directly as List(Integer).)")
        .def("set_qubit_readout_error", &qb::NoiseModel::set_qubit_readout_error, R"(
        ...
        .def("add_qubit_connectivity", &qb::NoiseModel::add_qubit_connectivity, R"(
        ...
        .def_property_readonly("connectivity", &qb::NoiseModel::get_connectivity, R"(Get connectivity as a list of connected qubit pairs)")
        .def_property_readonly("qobj_compiler", &qb::NoiseModel::get_qobj_compiler, R"(The name of the QObj compiler to use with the AER simulator)")
        .def_readwrite("name", &qb::NoiseModel::name, R"(The colloquial name of the noise model)");


  py::enum_<qb::NoiseModel::QubitConnectivity>(nm, "QubitConnectivity", R"(Type of qubit connectivity)")
