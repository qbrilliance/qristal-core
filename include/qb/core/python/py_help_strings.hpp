...
namespace qb {
  namespace help {
    ...
    const char* output_amplitudes_ = R"(
        output_amplitude:
    ...
    const char* out_raws_json_ = R"(
        out_raws_json:
    
        After calling session.run(), the counts from running sn shots are stored in session.out_raws_json, using a JSON format.
    )";
    
    /*const char* out_raws_map_ = R"(
        out_raws_map:
    
        After calling session.run(), the counts from running sn shots are stored in a dict of shot counts, with the keys given a lists of bits ordered according to qubit index.
    )";*/
