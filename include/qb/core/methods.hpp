...
public:
  Qbqe()
      : debug_qbqe_(false), name_m{{}}, number_m{{}}, infiles_{{}},
        include_qbs_{{"/mnt/qb/share/qblib.inc"}}, qpu_configs_{{"/mnt/qb/share/qpu_config.json"}},
        instrings_{{}}, irtarget_ms_{{{}}},
        accs_{{"tnqvm"}},
        aws_device_names_{{"DM1"}}, aws_formats_{{"openqasm3"}}, aws_verbatims_{{{false}}},
        aws_s3s_{{"amazon-braket-qbos"}}, aws_s3_paths_{{"output"}},
        aer_sim_types_{},
    ...
    xacc::Initialize();
    xacc::setIsPyApi();
    xacc::set_verbose(debug_qbqe_);
  }
  Qbqe(const std::string &name) : Qbqe() { name_m.push_back({name}); }
  Qbqe(const bool debug) : Qbqe() { debug_qbqe_ = debug; }

  // Setters and Getters
  // Notes:
  //  - setters and getters implementation should be defined in qbos_methods_getters_setters.cpp
  //  - help strings (Python) should be defined in qbos_methods_py_help_strings.cpp
  ...
  //
  void set_debug_qbqe(const bool &debug_qbqe);
  const bool &get_debug_qbqe() const;
  static const char *help_debug_qbqe_;
  //
