...
namespace qb
{
    namespace benchmark 
    {
        ...
        */
        ...
        double classical_fidelity(const std::map<Bitstring, size_t>& p, const std::map<Bitstring, size_t>& q) {
            double result = 0.0;
            //calculate total number of shots 
            size_t n_p = sumMapValues(p);
            size_t n_q = sumMapValues(q);
            
            //calculate fidelity
            for (auto const & [bitstring, counts] : p) {
                if ( q.contains(bitstring) ) {
                    result += std::sqrt(static_cast<double>(counts * q.at(bitstring)));
                }
            }
            return result * result / static_cast<double>(n_p * n_q);
        }
        /**
        ...
        */
        template <typename Bitstring>
        double classical_fidelity_to_uni(const std::map<Bitstring, size_t>& p, const size_t n_qubits) {
            double result = 0.0; 
        ...
        std::map<std::time_t, std::vector<double>> CircuitFidelity<WORKFLOW>::evaluate(const bool force_new) const {
            ...
            std::vector<std::vector<std::string>> ideal_bitcounts_collection = dlg.obtain_ideal_counts();
            ...
            for (const auto & [session_info, measured_bitcounts, ideal_bitcounts, timestamp] : std::ranges::views::zip(session_infos, measured_bitcounts_collection, ideal_bitcounts_collection, timestamps)) {
                ...
                for (const auto & [measured_bitcount, ideal_bitcount] : std::ranges::views::zip(measured_bitcounts, ideal_bitcounts)) {
                    const std::map<std::string, size_t>& p_meas = convert_to_counts_map(measured_bitcount, n_qubits);
                    const std::map<std::string, size_t>& p_ideal = convert_to_counts_map(ideal_bitcount, n_qubits);
                    double fc_ideal_meas = classical_fidelity(p_meas, p_ideal);
                    double fc_ideal_uni = classical_fidelity_to_uni(p_ideal, n_qubits); 
