        ...
        */
        class DataLoaderGenerator
        {
            public:
                ...
                * @return std::vector<std::vector<std::string>> a vector of bit strings (as std::string) for each stored time stamp.
                */
                std::vector<std::vector<std::map<std::vector<bool>, int>>> obtain_ideal_counts() const {
                    return load_data<BitCounts, std::vector<std::map<std::vector<bool>, int>>>(workflow_identifier_, "_ideal_"   , timestamps_);
                }
                /**
