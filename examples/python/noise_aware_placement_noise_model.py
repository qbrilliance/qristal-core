...
for qId in range(nb_qubits):
    ...
    if qId == 0:
        ...
        noise_model.add_gate_error(qb.core.DepolarizingChannel.Create(qId, 10*u1_error), "u1", [qId])
        noise_model.add_gate_error(qb.core.DepolarizingChannel.Create(qId, 10*u2_error), "u2", [qId])
        noise_model.add_gate_error(qb.core.DepolarizingChannel.Create(qId, 10*u3_error), "u3", [qId])
    else:
        noise_model.add_gate_error(qb.core.DepolarizingChannel.Create(qId, u1_error), "u1", [qId])
        noise_model.add_gate_error(qb.core.DepolarizingChannel.Create(qId, u2_error), "u2", [qId])
        noise_model.add_gate_error(qb.core.DepolarizingChannel.Create(qId, u3_error), "u3", [qId])
...
# Get the qb noise-aware placement pass (constructed with the above noise model)
placement = qb.core.noise_aware_placement_pass(noise_model)
