...
int main () {
    ...
    //qbOS::qaoa::Params params{qbOS::qaoa::makeJob(qbOS::qaoa::JobID::QAP_5_4)}; // has all inputs for qaoa
...
// interface should not be Pythonic, this is C++, use a proper struct and initialize it
    ...
    const qbOS::VectorString eigenstates{qaoa.get_out_eigenstates()}; //[0][0]
    const qbOS::VectorMapND energies{qaoa.get_out_energys()}; //[0][0][0]
