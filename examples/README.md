...
This folder contains a series of example C++ and Python main programs.
...
`amplitude_amplification`
  ...
  - `amp_amplification_iteration.py`  
  qubits: 1  
  circuit depth: 6318  
  noise: false  
...
`amplitude_estimation`
  ...
  - `canonical_amplitude_estimation.py`  
  qubits: 1 + number of precision qubits (user defined)  
  noise: false  
  ...
  - `canonical_amplitude_estimation_oracle.py`  
  qubits: 9  
  noise: false  
  ...
  - `ML_amplitude_estimation_parameter_testing.py`  
  qubits: 1  
  noise: false  
...
qubits: 9 (comparator), 2 (comparator_as_oracle)  
gate depth: 226 (comparator), 113 (comparator_as_oracle)  
noise: false  
...
qubits: 14  
gate depth: 3392 (with ancilla), 10245 (without ancilla)  
noise: false  
...
Without ancilla: 
qubits: 9 (with ancilla), 7 (without ancilla)  
gate depth: 106 (with ancilla), 92 (without ancilla)  
noise: false  
...
A commandline interface to the QB SDK.  A simple invocation after compiling the CLI is:
```
./qbsdkcli -q2 --random=2
```
which will run a random circuit on 2 qubits, with circuit depth of 2. Further details can be found in [cpp/qbsdkcli/README.md](cpp/qbsdkcli/README.md)
