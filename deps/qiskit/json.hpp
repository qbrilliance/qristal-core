...
inline json_t JSON::load(std::string name) {
  if (name == "") {
    json_t js;
...
inline bool JSON::check_key(std::string key, const json_t &js) {
...
inline bool JSON::check_keys(std::vector<std::string> keys, const json_t &js) {
  bool pass = true;
  for (auto s : keys)
    pass &= check_key(s, js);
  return pass;
